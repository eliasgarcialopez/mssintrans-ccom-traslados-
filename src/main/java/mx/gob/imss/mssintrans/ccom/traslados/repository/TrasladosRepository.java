...
public interface TrasladosRepository extends JpaRepository<TrasladosEntity, Integer> {
	...
	@Query(value = ""
			+ "SELECT	SST.*, SAS.NOM_UNIDAD_ADSCRIPCION AS NOM_UNIDAD_SOLICITANTE, SAA.NOM_UNIDAD_ADSCRIPCION AS NOM_UNIDAD_ADSCRIPCION, SAD.NOM_UNIDAD_ADSCRIPCION AS NOM_UNIDAD_DESTINO,"
			...
			+ "INNER 	JOIN SINTRANSC_UNIDADES_ADSCRIPCION SAD ON SAD.ID_UNIDAD_ADSCRIPCION = SST.CVE_DESTINO         "
	...
	@Query(value = ""
			...
			+ "LEFT 	JOIN SINTRANST_CENSO_DOCTORES SDA ON SDA.CVE_MATRICULA_DOCTOR = SST.CVE_MATRICULA_DOCTOR_AUTORIZA   "			
			+ "LEFT 	JOIN SINTRANSC_ENTIDADES SE ON SE.ID_ENTIDAD = SM.ID_ENTIDAD                                        "			
			+ "WHERE   	SST.IND_ACTIVO 	= '1'  AND SAS.ID_UNIDAD_ADSCRIPCION = ?     AND SST.DES_ESTATUS_SOLICITUD IN ('1','2','3')                                                                       "
			+ "",
			countQuery =""
					+ "SELECT	COUNT(SST.ID_SOLICITUD) "
					+ "FROM		SINTRANST_SOLICITUD_TRASLADO SST   									 "
					...
					+ "",
			nativeQuery = true )
	Page<TrasladosEntity>consultaGeneralUnidad(Pageable paginado, Integer idUnidadAdscripcion);
